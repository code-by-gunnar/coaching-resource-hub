  ...
  <div class="admin-container">
    ...
    <div v-else-if="!hasAdminAccess" class="admin-content">
      <div class="admin-header">
        ...
        <p class="admin-subtitle">You need administrator privileges to edit performance analysis.</p>
      </div>
      <ActionButton href="/docs/admin/" variant="secondary" icon="←">Back to Admin Hub</ActionButton>
    </div>

    <!-- Competency Performance Analysis Editor -->
    <div v-else class="admin-content">
      <div class="admin-header">
        <div class="header-main">
          <h1>{{ analysisId ? 'Edit' : 'Create' }} Performance Analysis</h1>
          <p class="admin-subtitle">{{ analysisId ? 'Update' : 'Create' }} competency performance analysis</p>
        </div>
      </div>
      ...
      <nav class="breadcrumb">
        ...
        <span class="breadcrumb-separator">→</span>
        <span>{{ analysisId ? 'Edit' : 'Create' }} Performance Analysis</span>
      </nav>
      ...
      <div class="editor-container">
        <div class="form-card">
          <div class="section-header">
            <div>
              <h2>📊 Performance Analysis Fields</h2>
              <p class="section-description">Performance analysis data from relational database</p>
            </div>
            <div class="table-info">
              <span class="table-badge">competency_performance_analysis</span>
            </div>
          </div>

          <form @submit.prevent="saveAnalysis" class="editor-form">
            ...
            <div class="form-group">
              ...
              <div class="field-help">Which competency this analysis belongs to</div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="field-help">Assessment level this analysis applies to</div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="field-help">Performance scoring tier (optional)</div>
            </div>

            <!-- Analysis Text (Required) -->
            <div class="form-group">
              <label class="form-label">
                <span style="color: #dc2626; font-weight: 600;">Analysis Text</span>
                <span class="field-info">analysis_text (text, required)</span>
              </label>
              <textarea 
                v-model="formData.analysis_text"
                rows="6"
                class="form-textarea"
                :class="{ error: errors.analysis_text }"
                placeholder="Detailed analysis of performance at this level..."
                required
              ></textarea>
              <div v-if="errors.analysis_text" class="error-message">{{ errors.analysis_text }}</div>
              <div class="field-help">Detailed performance analysis text (required)</div>
            </div>
            ...
            <div class="form-group">
              ...
              >
              <div class="field-help">Order for displaying this analysis (0 = first, 100 = last)</div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="field-help">Whether this analysis is used in assessments</div>
            </div>

            <!-- Metadata (Read-only) -->
            <div v-if="analysisId" class="form-group metadata-group">
            ...
            <div class="form-actions">
              ...
              <ActionButton 
                ...
                :loading="saving"
              >
                {{ analysisId ? 'Update' : 'Create' }} Performance Analysis
              </ActionButton>
            </div>
          </form>
        </div>
        ...
        <div class="info-card">
          ...
          <div class="info-content">
            ...
            <ul class="related-links">
              <li><strong>competency_performance_analysis</strong> → Main table</li>
              ...
              <li><strong>scoring_tiers</strong> → Scoring tier lookup</li>
            </ul>
...
// Get analysis ID from URL params using client-side approach
...
const isFormValid = computed(() => {
  return formData.value.competency_id && 
         ...
         formData.value.analysis_text?.trim() &&
...
const validateField = (field) => {
  ...
  switch (field) {
    ...
    case 'analysis_text':
      if (!formData.value.analysis_text?.trim()) {
        errors.value.analysis_text = 'Analysis text is required'
      }
      break
  }
}
...
const loadReferenceData = async () => {
  ...
  try {
    ...
    if (tierError) {
      ...
      // It's OK if this fails, scoring_tier_id is optional
    } else {
...
const loadAnalysis = async () => {
  if (!analysisId.value || !adminSupabase) return
  ...
  try {
    ...
    if (data) {
      ...
      console.log('Loaded analysis data:', formData.value)
    }
  } catch (error) {
    console.error('Error loading analysis:', error)
...
const saveAnalysis = async () => {
  ...
  try {
    ...
    const dbData = {
      ...
      analysis_text: formData.value.analysis_text?.trim(),
    ...
    alert('Performance analysis saved successfully!')
    goBack()
  } catch (error) {

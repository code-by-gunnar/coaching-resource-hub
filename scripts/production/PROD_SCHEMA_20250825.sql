-- PUBLIC SCHEMA ESSENTIALS ONLY
-- No privileges, no permissions, just tables and functions
...
CREATE FUNCTION public.auto_calculate_score() RETURNS trigger
...
CREATE FUNCTION public.calculate_assessment_score(attempt_uuid uuid) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO ''
    AS $$
DECLARE
...
CREATE FUNCTION public.calculate_enriched_assessment_data(attempt_uuid uuid) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO ''
    AS $$
DECLARE
...
CREATE FUNCTION public.calculate_section_progress(p_workbook_id uuid, p_section_number integer) RETURNS integer
    LANGUAGE plpgsql SECURITY DEFINER
...
CREATE FUNCTION public.calculate_section_progress(user_id_param uuid, section_id_param uuid) RETURNS integer
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE FUNCTION public.check_answer_correctness_v4() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
CREATE FUNCTION public.check_enriched_data_status(attempt_uuid uuid) RETURNS boolean
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO ''
    AS $$
DECLARE
...
CREATE FUNCTION public.cleanup_expired_pdfs() RETURNS integer
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE FUNCTION public.cleanup_temporary_pdfs() RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO ''
    AS $$
BEGIN
...
CREATE FUNCTION public.get_analysis_type_for_score(score_percentage integer, framework_code text DEFAULT NULL::text, assessment_level_code text DEFAULT NULL::text) RETURNS text
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE FUNCTION public.get_competency_consolidated_insights(p_competency_id uuid, p_framework_id uuid, p_assessment_level_id uuid) RETURNS TABLE(analysis_type_code text, performance_insight text, development_focus text, practical_application text)
    LANGUAGE plpgsql
    SET search_path TO 'public'
    AS $$
BEGIN
...
CREATE FUNCTION public.get_consolidated_insights_for_pdf(p_competency_id uuid, p_framework_id uuid, p_assessment_level_id uuid, p_analysis_type_id uuid) RETURNS TABLE(performance_insight text, development_focus text, practical_application text)
    LANGUAGE plpgsql
    SET search_path TO 'public'
    AS $$
BEGIN
...
CREATE FUNCTION public.get_enriched_assessment_data(attempt_uuid uuid) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO ''
    AS $$
DECLARE
...
CREATE FUNCTION public.get_learning_paths_for_assessment(weak_competency_areas text[], overall_score integer) RETURNS TABLE(category_id uuid, category_title text, category_description text, category_icon text, resources json[])
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO ''
    AS $$
BEGIN
...
CREATE FUNCTION public.get_learning_paths_for_assessment(weak_competency_areas text[], overall_score integer, assessment_level text) RETURNS TABLE(category_id uuid, category_title text, category_description text, category_icon text, resources json[])
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO ''
    AS $$
DECLARE
...
CREATE FUNCTION public.get_or_create_user_workbook(user_id_param uuid) RETURNS TABLE(id uuid, user_id uuid, progress_data jsonb, created_at timestamp with time zone, updated_at timestamp with time zone)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE FUNCTION public.get_scoring_tier_basic(score_percentage integer) RETURNS TABLE(tier_code character varying, tier_name character varying, score_min integer, score_max integer)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
CREATE FUNCTION public.get_user_best_score(user_uuid uuid, assessment_uuid uuid) RETURNS integer
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO ''
    AS $$
DECLARE
...
CREATE FUNCTION public.initialize_user_workbook(user_id_param uuid) RETURNS uuid
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE FUNCTION public.is_weakness_score(score_percentage integer, framework_code text DEFAULT NULL::text, assessment_level_code text DEFAULT NULL::text) RETURNS boolean
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE FUNCTION public.store_frontend_insights(attempt_uuid uuid, frontend_insights jsonb) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO ''
    AS $$
BEGIN
...
CREATE FUNCTION public.update_attempt_status() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO ''
    AS $$
BEGIN
...
CREATE FUNCTION public.update_workbook_progress_timestamp() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
CREATE FUNCTION public.update_workbook_response_timestamp() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
CREATE VIEW public.assessment_overview WITH (security_invoker='true') AS
 SELECT a.id,
    a.slug,
    a.title,
    a.description,
...
CREATE TABLE public.competency_consolidated_insights (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.competency_display_names (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.competency_leverage_strengths (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.competency_performance_analysis (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    user_id uuid,
...
CREATE TABLE public.competency_rich_insights (
    id uuid,
...
CREATE TABLE public.competency_strategic_actions (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.framework_scoring_overrides (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    framework_id uuid NOT NULL,
    assessment_level_id uuid,
...
CREATE TABLE public.learning_path_categories (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.learning_resource_competencies (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.scoring_tiers (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    tier_code character varying(20) NOT NULL,
...
CREATE VIEW public.leverage_strengths_with_analysis_type WITH (security_invoker='true', security_barrier='true') AS
 SELECT cls.id,
    cls.leverage_text,
    cls.priority_order,
    cls.competency_id,
...
CREATE VIEW public.performance_analysis_with_analysis_type WITH (security_invoker='true', security_barrier='true') AS
 SELECT cpa.id,
    cpa.analysis_text,
    cpa.sort_order,
    cpa.competency_id,
...
CREATE VIEW public.rich_insights_compatibility WITH (security_invoker='true') AS
 SELECT id,
    competency_id,
    framework_id,
    assessment_level_id,
...
CREATE TABLE public.self_study (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    user_id uuid NOT NULL,
    name text NOT NULL,
...
CREATE VIEW public.strategic_actions_with_analysis_type WITH (security_invoker='true', security_barrier='true') AS
 SELECT csa.id,
    csa.action_text,
    csa.priority_order,
    csa.sort_order,
...
CREATE TABLE public.tag_actions (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.tag_insights (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    skill_tag_id uuid NOT NULL,
...
CREATE TABLE public.temporary_pdf_files (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE VIEW public.user_assessment_progress WITH (security_invoker='true') AS
 SELECT ua.user_id,
    ua.assessment_id,
...
CREATE VIEW public.user_competency_performance WITH (security_invoker='true') AS
 SELECT ua.user_id,
...
CREATE VIEW public.v_assessment_responses WITH (security_invoker='true') AS
 SELECT uqr.id,
    uqr.attempt_id,
    uqr.question_id,
    aq.question_order,
...
CREATE TABLE public.workbook_field_definitions (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE INDEX idx_learning_resources_analysis_type_id ON public.learning_resources USING btree (analysis_type_id);
...
CREATE TRIGGER trigger_auto_calculate_score AFTER UPDATE ON public.user_assessment_attempts FOR EACH ROW EXECUTE FUNCTION public.auto_calculate_score();


--
--

CREATE TRIGGER trigger_check_answer_correctness_v4 BEFORE INSERT OR UPDATE ON public.user_question_responses FOR EACH ROW EXECUTE FUNCTION public.check_answer_correctness_v4();


--
--

CREATE TRIGGER trigger_update_attempt_status AFTER INSERT ON public.user_question_responses FOR EACH ROW EXECUTE FUNCTION public.update_attempt_status();


--
--

CREATE TRIGGER update_workbook_progress_updated_at BEFORE UPDATE ON public.workbook_progress FOR EACH ROW EXECUTE FUNCTION public.update_workbook_progress_timestamp();


--
--

CREATE TRIGGER update_workbook_responses_updated_at BEFORE UPDATE ON public.workbook_responses FOR EACH ROW EXECUTE FUNCTION public.update_workbook_response_timestamp();
...
ALTER TABLE ONLY public.learning_resources
    ADD CONSTRAINT learning_resources_analysis_type_id_fkey FOREIGN KEY (analysis_type_id) REFERENCES public.analysis_types(id);
